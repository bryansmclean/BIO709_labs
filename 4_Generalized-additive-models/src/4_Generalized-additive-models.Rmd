...
You can load the **beaver1** dataset as follows. Be sure to use only data from one day, as the temperature data were collected intermittently for multiple days.
...
-- In this plot, make sure to include standard error for the GAM fit (which is done automatically. In addition, apply shading to the standard error and change the color of the shading to something other than gray. See ?plot.gam() for the correct arguments for specifying this.\
...
\
-- The default in plot.gam() is to plot only the smooth term(s). However, you can use the **all.terms** argument in this function to produce plots of non-smoothed terms as well. Plot this term, and look at the slope here and back in the model summary. Does activity have a positive or negative effect on body temperature?? \
-- Save this plot as an image (preferably .pdf) and name this file according to our convention listed in Canvas assignments.\
\

**4. Compare the beaver temp GAM to a linear model with polynomial terms.**\
\
-- Remember that basis expansion using ordered polynomials can be a good fit to some data, but even very high-order polynomials may not work for other data types. Construct a multiple linear model (using lm()) that predicts beaver temperature using a combination of a) a 3rd order polynomial on time, and b) activity.\
-- Do the same, but this time using two higher-order polynomials on time. Use **degree = 5** and **degree = 7** for this.\
-- Summarize the three linear model objects above using the summary() function, and choose the model with the **lowest (most negative)** AIC score. Use the capture.output() function to save this full model SUMMARY as a .txt file. Remember to name this file according to our convention listed in Canvas assignments.\
\

**5. Plot the GAMs and all 3 LMs, along with empirical data.** \
\
-- The plot should be bivariate, with time (X) and temperature (Y) from your model.\
